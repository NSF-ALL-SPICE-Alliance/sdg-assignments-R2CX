...
```
```{r}
importance_df <- as.data.frame(rf_slavery$importance)
```
```{r}
...
ggplot(importance_df_top_10, aes( x = `%IncMSE`, y = reorder(variable, `%IncMSE`))) +
  geom_bar(stat = "identity", fill = "STeelblue", color = "black" ) + 
  theme_minimal() + 
  labs(title = "Important Variables In Slavery",
       subtitle = "Top 10",
       y = "SDG Indicator",
       x = "Feature Importance (% Increase in Mean Squared Error)")
```
...
```{r}
pdp::partial(rf_slavery, pred.var = "normalized_score_sdg8_slavery", plot = TRUE)
```
```{r}
pd <- pdp::partial(rf_slavery, pred.var = c("normalized_score_sdg8_slavery", "normalized_score_sdg9_roads"))
plotPartial(pd)
```
