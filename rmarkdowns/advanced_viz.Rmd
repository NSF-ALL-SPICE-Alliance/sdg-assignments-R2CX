...
ggplot(sdr_data, aes(x = goal_6_score, fill=regions_used_for_the_sdr)) +
  ...
  theme_minimal() +
  scale_fill_viridis_d(option = "inferno") +
  labs(title = "Distributions of SDG 6 Scores",
       x = "SDG 6 Score",
       y = "Number of Countries",
       fill = "Region")
```
...
```{r}
goal_3_4_scatterplot <- ggplot(data = sdr_data, aes(x= goal_3_score, y = goal_4_score, color = regions_used_for_the_sdr)) +
  geom_point() +
  theme_minimal() +
...
ggplotly(goal_3_4_scatterplot)
```
```{r}
goal_3_histogram <- ggplot(data = sdr_data, aes(x= goal_3_score, y = goal_4_score, color = regions_used_for_the_sdr)) +
  geom_point() + 
  theme_minimal() + 
...
# ```{r}
# \Users\Student\AppData\Local\Temp\RtmpSQST82\downloaded_packages
# 
# ```
# ```{r}
# library <- installed.packages (rnaturalearth) C:\Users\Student\AppData\Local\Temp\RtmpSQST82\downloaded_packages
# ```
...
```{r}
mytext <- paste("Country: ", sdr_data_joined$country, "<br/>", "Goal 3 Score: ", round(sdr_data_world_joined$goal_3_score, 2), sep="")%>% 
  lapply(htmltools::HTML)

leaflet(sdr_data_world_joined) %>% 
  addTiles() %>%
  setView( lat=10, lng=0, zoom=2)%>% 
  addPOlygons(stroke = FALSE, fili0pacity = 0.5, smooyhFActor = 0.5, color = ~colorQuantile("PuRd", goal_3_score)(goal_7_score), label = mytext)
```
```{r}
mytext <- paste(
    "Country: ", sdr_data_world_joined$country,"<br/>", 
...
leaflet(sdr_data_world_joined) %>% 
  addTiles()  %>% 
  setView( lat=10, lng=0 , zoom=2) %>%
  addPolygons(stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, color = ~colorQuantile("PuRd", goal_7_score)(goal_7_score), label = mytext)
```
